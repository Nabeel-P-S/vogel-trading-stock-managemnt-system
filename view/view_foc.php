...
$to_date=$date;
$sales_sql = "SELECT foc.profit,foc.paid,foc.invoice_no,foc.foc,foc.foc_id,foc.customer_id,foc.foc_date,customers.customer_name,branches.branch_name,staffs.staff_name,staffs.staff_id,branches.branch_id from foc 
    LEFT join customers on customers.customer_id=foc.customer_id 
     
    LEFT join staffs on staffs.staff_id=foc.staff_id 
    LEFT join branches on branches.branch_id=staffs.branch_id  order by foc_id desc ";
...
$sales_sql = "SELECT sales.profit,sales.paid,sales.invoice_no,sales.sales,sales.foc_id,sales.customer_id,sales.sales_date,customers.customer_name,branches.branch_name,staffs.staff_name,staffs.staff_id,branches.branch_id from sales 
    LEFT join customers on customers.customer_id=sales.customer_id 
     
    LEFT join staffs on staffs.staff_id=sales.staff_id 
    LEFT join branches on branches.branch_id=staffs.branch_id  where sales.$name ='$value' AND sales.category='1' order by foc_id desc";
    } 
...
<!-- <a class="btn btn-primary" href="../view/full_sales.php"> Full SALES</a> -->
    ...
    <div id="printableArea"  class="col-md-12" style="overflow: auto; ">
      <h3 style="text-align: center;"><b>FOCL IST   </b></h3>
...
?>
   <tr class="table_row" style="cursor: pointer;">
     <!-- <td onclick= "open_size_list('<?php echo $foc_id;?>');"> <?php echo $foc_id; ?> </td> -->
         <td ><a> <?php echo $foc_id; ?> </a></td>
<!-- onclick="delete_sale('<?php echo $foc_id;?>')" -->
    ...
    <td onclick="refresh_again1('<?php echo $customer_id;?>','customer_id')"> <?php echo $customer_name; ?> </td>
     ...
     <td><a >
       ...
       <table  ><tr>
          ...
          <?php
              $query2=mysqli_query($conn,"SELECT foc_articles.foc_id, foc_articles.article_id,foc_articles.article_qty,articles.article_no,articles.article_price,articles.sales_price, foc_articles.id,articles.article_name FROM `foc_articles` left join `articles` on articles.article_id=foc_articles.article_id  WHERE foc_articles.foc_id='$foc_id'");
                 $sleno=1;
             $cost_total=0;
             $mrp_total=0;
...
$margin=$mrp_total-$cost_total;
    ...
    ?></tr>
     ...
     <td> <a 
     href="../print/foc_bill.php?foc_id=<?php echo $foc_id;?>"> <?php echo $sales; ?> </a></td>
     <td> <a href="../print/delivery_order.php?foc_id=<?php echo $foc_id;?>"> <?php echo $profit; ?> </a></td>
     <td > <a href="../edit/sales_edit.php?foc_id=<?php echo $foc_id;?>"><?php echo $paid; ?></a></td>
  ...
  <td colspan="9" style="padding: 0;">
     <div style="display: none;" id="size_td<?php echo $foc_id; ?>">
   ...
   <tr >
           ...
           <td style="text-align: center;"> ARTICLE PRICE </td>
    ...
    <?php
              $query2=mysqli_query($conn,"SELECT sales_articles.foc_id, sales_articles.article_id,sales_articles.article_qty,articles.article_no,articles.sales_price, sales_articles.id,articles.article_name FROM `sales_articles` left join `articles` on articles.article_id=sales_articles.article_id  WHERE sales_articles.foc_id='$foc_id'");
                 $sleno=1;
                while ($fetch2=mysqli_fetch_array($query2))
                 {
                 $id=$fetch2['id'];
...
?>
  <tr class="table_summary" >    
      ...
      <td >TOTAL </td>
      <td colspan="1">ORDERS : <?php echo $count; ?> </td>
...
<script type="text/javascript">
  function open_size_list(foc_id)
  {

if (document.getElementById("size_td"+foc_id).style.display=="block")
{
  document.getElementById("size_td"+foc_id).style.display="none";
  }
  else
  {
   document.getElementById("size_td"+foc_id).style.display="block"; 
  }
}
</script>
<script type="text/javascript">
